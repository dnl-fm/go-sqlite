...
Turso 0.6.1 is a correctness release on top of the 0.6.0 engine changes. That
is exactly when a lab earns its rent.
...
Through `tursogo v0.6.1` on this Linux host, concurrent child processes still
...
`tursodb 0.6.1` binary, the lab checks whether a Go app holding the database
open using `experimental=multiprocess_wal` can be inspected and written by the
CLI with `--experimental-multiprocess-wal`.
...
```bash
TURSO_V061_TURSODB_BIN=/path/to/tursodb GOWORK=off go test ./...
```
